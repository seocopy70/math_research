...
\]
with odd $p$, $f\ge1$, and even $d\ge2$, every affine representation factors through
...
These are different questions. The first is a factorization problem in a specified representation category. The second is a recognition problem whose answer depends on the input category and predicate.

\begin{theorem}[Sharp affine finite window]
Let $p$ be odd, $f\ge1$, $d\ge2$ even, and let $G_{f,d}$ be as above. For every $k\ge2$, every
\[
\psi=(z,\rho):G_{f,d}\longrightarrow S_k
\]
with $z\in Z^1(G_{f,d},A_k(\rho))$ factors uniquely through
\[
...
\begin{proof}
Use the Jennings--Lazard product
\[
...
For every pro-$p$ group $G$, every $\rho:G\to U_{1,k}$, and every $z\in Z^1(G,A_k(\rho))$, the homomorphism
\[
\psi(g)=(z(g),\rho(g))
\]
...
Coboundaries are transported by the same factorization.
\enden{proof}
...
\boxed{n_{\mathrm{aff}}(k)=p^{k-1}+1}
\]
for the category of all crossed-cocycle representations into $A_k(\rho)\rtimes U_{1,k}$.
\enden{theorem}
...
For $f<k$, the abelianization retains $p^f[x_1]=0$, so the finite torsion order remains visible.

\section{Factorization is not recognition}
For a candidate $\rho:Q_k\to U_{1,k}$ define
\[
...
Let $F_d$ be free pro-$p$ of finite rank $d\ge2$. Every orientation $\rho:F_d\to U_{1,k}$ is Kummerian at every finite coefficient level. Hence
$\mathsf K_k(F_d/P_{p^{k-1}+1}(F_d),\rho)$ does not single out a unique orientation.
\enden{proposition}
...
vanishes and the $H^1$ reduction map is surjective. Factorization transfers the same lifting information to the finite quotient. Since $F_d$ has many distinct orientations, the predicate itself cannot be a uniqueness selector on this class.
\enden{proof}
...
Let $G=D_1*_p\cdots *_pD_m$ be a finite free pro-$p$ product of Demushkin groups, each with canonical orientation. Write $T_n(G)=G/P_n(G)$.
...
For a finite free pro-$p$ product of Demushkin groups, every affine crossed-cocycle representation at level $k$ factors through
\[
G/P_{p^{k-1}+1}(G).
\]
...
The canonical Demushkin orientation and its Kummerian characterization are classical \cite{Labute1967,EfratQuadrelli2019}. Oriented elementary-type constructions and related Kummerian closure phenomena are treated by Quadrelli--Weigel \cite{QuadrelliWeigel2022}. Quadrelli's 2024 work studies 1-cyclotomicity and quotient-related obstructions \cite{Quadrelli2024}. These are surrounding results, not claims of prior identity with the present theorem.
