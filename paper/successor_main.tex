...
\]
Let $e(n)=\lceil\log_p n\rceil$. From the Jennings--Lazard product,
after abelianization only the $i=1$ power terms can contribute, so the
image of $P_n(D_{f,d})$ in $D_{f,d}^{\mathrm{ab}}$ is contained in
$p^{e(n)}D_{f,d}^{\mathrm{ab}}$. At
$n=p^{k-1}+1$ one has $e(n)=k$, hence the image of the finite-window
kernel is contained in $p^kD_{f,d}^{\mathrm{ab}}$. Since $f<k$, this
subgroup is zero on the $\mathbb Z/p^f\mathbb Z$ summand. Therefore the
order-$p^f$ torsion class represented by $x_1$ survives unchanged in the
