...
keys_b1 = [(1,0),(1,1),(1,2)]
ranks_b1 = {k: int(rank3(images[k])) for k in keys_b1}
print("B1 transports =", keys_b1)
print("individual ranks =", ranks_b1)
pairwise_b1 = {}
for i,k1 in enumerate(keys_b1):
    for k2 in keys_b1[i+1:]:
        jr = int(rank3(column_basis(np.column_stack([images[k1], images[k2]]))))
        pairwise_b1[f"{k1}={k2}"] = {"join_rank": jr, "equal": jr == ranks_b1[k1] == ranks_b1[k2]}
print("pairwise equality =", pairwise_b1)
