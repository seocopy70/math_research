...
def rank3(A):
    ...
    for c in range(n):
        ...
        r+=1
    return r
Q=np.zeros((45,0),dtype=int);rk=0
for j in range(45):
...
for i in range(45):
    ...
    if rr==45:break
QB9=np.empty((45,45),dtype=object)
for i in range(45):
    for j in range(45):QB9[i,j]=f9(Q[rowsQ[i],j])
QB9inv=inv9(QB9);Q9=np.empty((45,45),dtype=object)
for i in range(45):
    for j in range(45):Q9[i,j]=f9(Q[i,j])
...
def scalar_on_line(A,v):
    Av=mv(A,v);i=next(i for i,x in enumerate(v) if not fzero(x));lam=fdiv(Av[i],v[i]);assert all(fzero(fsub(Av[j],fmul(lam,v[j]))) for j in range(len(v)));return lam
chi={(a,b):scalar_on_line(quotient_action9(torus(a,b)),v) for a in F9star for b in F9star}
for a in F9star:
    for b in F9star:
        for c in F9star:
            for d in F9star:assert chi[(fmul(a,c),fmul(b,d))]==fmul(chi[(a,b)],chi[(c,d)])
...
print('PHASE 2-14 / EXACT F9 TORUS PROBE');print('|F9*| =',len(F9star));print('dim E = 35');print('dim E^{U+(F9)} =',len(fixed));print('character matches exponents (e1,e2) mod 8 =',matches);print('EXPECTED (2,1) = (3,1) in epsilon coordinates');print('EXPECTED character a^3 b =',(3,1) in matches);print('CERTIFICATE: exact F9 torus character law verified on all 64 torus elements.')
