...
# Reconstruct W_d from the authoritative degree-4 action.
...
# K = ker(N) inside W, transported to ambient L4 coordinates.
def nullspace3(M):
...
# Independent intersection check: dim(W cap W_d)=35 and K subset W_d.
assert rank3(np.column_stack([Wmat, wd_mat])) == 55
assert rank3(np.column_stack([Kamb, wd_mat])) == 45

# Extend K to an ambient 60D basis using the authoritative L4 basis B.
C = Kamb.copy(); rankC = rank3(C)
...
for i in range(256):
    ...
    nr=rank3(cand)
    if nr>rr: rows.append(i); Rsel=cand; rr=nr
    if rr==60: break
assert rr == 60

# Express C in the authoritative B=[R4,W] coordinates.
Bsel=B[rows,:]
Binv=inverse3(Bsel)
C_in_B=(Binv @ C[rows,:])%P
C_in_B_inv=inverse3(C_in_B)

# Quotient L4/K action. Python uses columns; quotient is last 25 coordinates.
Qgens=[]
for gA in A:
...
def independent_columns(M):
    M=np.array(M,dtype=np.int64)%P
    cur=np.empty((M.shape[0],0),dtype=np.int64); r=0; out=[]
    for j in range(M.shape[1]):
        cand=np.column_stack([cur,M[:,j]])
        nr=rank3(cand)
        if nr>r: out.append(j); cur=cand; r=nr
    return out,cur
_,WQ10=independent_columns(WQ)
assert WQ10.shape == (25,10)

# GAP uses row/right action, hence transpose the authoritative column matrices.
def gap_rows(M):
...
parts=[x.strip() for x in line.split('=',1)[1].strip().strip('[]').split(',') if x.strip()]
if not parts or any(int(x)!=10 for x in parts):
    raise SystemExit('Not all minimal submodules of quotient are 10D; broader lattice analysis required')
