# A3-4-6: quotient W_d / I identification.
import runpy
import numpy as np
...
def intertwiner_data(A_src, A_tgt):
    ...
    nb = null3(np.column_stack(cols)); max_rank = 0; full = None
    for x in nb:
...
# Reuse the verified A3-4-5 construction and its exact quotient W45/I.
...
B = I_in_Wd.copy(); Qcols = []; r = rank3(B)
for j in range(45):
...
S = np.column_stack([I_in_Wd, Q]); assert rank3(S) == 45
...
print('dim W45 =', rank3(W)); print('dim Wd =', rank3(Wd_basis)); print('dim I =', rank3(I_W)); print('dim(Wd/I) =', 10)
print('\nTEST 1: Wd/I ~= Sym^2(V)')
print('dim Hom_H(Wd/I, Sym^2(V)) =', hom_d); print('maximum intertwiner rank =', maxrank_d); print('FULL_RANK_INTERTWINER_FOUND =', P_d is not None)
print('\nTEST 2: Wd/I ~= W45/I')
print('dim Hom_H(Wd/I, W45/I) =', hom_cross); print('maximum intertwiner rank =', maxrank_cross); print('FULL_RANK_INTERTWINER_FOUND =', P_cross is not None)
print('\nCERTIFICATES')
print('WD_OVER_I_EQUALS_SYM2 =', P_d is not None); print('WD_OVER_I_EQUALS_W45_OVER_I =', P_cross is not None)
assert P_d is not None and P_cross is not None
