...
AnalyzeModule := function(name,M)
  local soc,rad,nsoc,nrad,cur,b,nb,curdim,rad_dims,soc_dims,comp,collected;
  Print("MODULE = ",name,"\n");
  Print("INDECOMPOSABLE = ",MTX.IsIndecomposable(M),"\n");
  soc := MTX.BasisSocle(M); nsoc := Length(soc);
  Print("SOCLE_DIM = ",nsoc,"\n");
  rad := MTX.BasisRadical(M); nrad := Length(rad);
  Print("RADICAL_DIM = ",nrad,"\n");
  Print("TOP_DIM = ",MTX.Dimension(M)-nrad,"\n");
  rad_dims := [MTX.Dimension(M)]; cur := M;
  while MTX.Dimension(cur) > 0 do
    b := MTX.BasisRadical(cur); nb := Length(b); Add(rad_dims,nb);
  ...
  soc_dims := [0]; cur := M; curdim := MTX.Dimension(cur);
  while curdim > 0 do
    b := MTX.BasisSocle(cur); nb := Length(b);
    Add(soc_dims,MTX.Dimension(M)-curdim+nb);
    if nb = curdim then break; fi;
    b := MTX.NormedBasisAndBaseChange(b)[1];
    cur := MTX.InducedActionFactorModule(cur,b); curdim := MTX.Dimension(cur);
  od;
  ...
  return rec(soc_dim:=nsoc,rad_dim:=nrad,rad_dims:=rad_dims,soc_dims:=soc_dims,
             comp_dims:=List(comp,MTX.Dimension),indecomp:=MTX.IsIndecomposable(M));
end;;
