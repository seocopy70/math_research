...
## 2026-09-25 — KERNEL-CONTAINED COUNTEREXAMPLE CLOSED

Set N = P_3 intersect ker(chi). The previous x_2^3/mod-27 argument is discarded as unnecessary and is superseded by a cleaner commutator argument.

1. N is closed, normal, and N is contained in ker(chi) intersect P_3.
2. Let P_2 be the second Zassenhaus term. Since chi is a homomorphism to an abelian group, [P_2,G] is contained in ker(chi). By the Zassenhaus commutator rule, [P_2,G] is contained in P_3. Hence [P_2,G] is contained in N.
3. We need [P_2,G] not contained in P_4. In the associated restricted graded Lie algebra, degree 2 is generated by the degree-2 commutators modulo the single initial Demushkin relation R=[X_1,X_2]+[X_3,X_4]. The degree-3 relation space is the span of [R,X_i], i=1,...,4. The element [[X_1,X_2],X_1] is not in that span. An explicit coefficient check in the free associative degree-3 space over F_3 gives rank(span{[R,X_i]})=4, while adjoining [[X_1,X_2],X_1] raises the rank to 5. Since the p=3 restricted p-power degree-3 summands are separate from the ordinary Lie degree-3 part, this proves [[X_1,X_2],X_1] survives in L_3(G). Therefore [P_2,G] is not contained in P_4.
4. Thus N/P_4 is nonzero. Since N^3 is contained in P_9 and [N,G] is contained in [P_3,G] contained in P_4, we have N^3[N,G] contained in P_4. Hence N/N^3[N,G] is nonzero.
5. Because N is contained in P_3, and P_3 is contained in Phi(G), the inclusion-induced map N/N^3[N,G] -> G/Phi(G) is zero. It is therefore not injective. By the duality in Quadrelli Prop. 2.10, res^1_{G,N}: H^1(G,F_3) -> H^1(N,F_3)^G is not surjective.

This is now a genuine counterexample satisfying the kernel hypothesis of Proposition 2.10. Therefore the proposed automatic application of Prop. 2.10 to natural Zassenhaus-level kernel subgroups is closed negatively.

### Classification update
- N=P_3 intersect ker(chi) admissibility: PASS / CLOSED.
- N/N^3[N,G] nonzero: PASS / CLOSED, via the degree-3 restricted-Lie calculation above.
- Restriction nonsurjectivity for this admissible N: PASS / CLOSED.
- Prop. 2.10 automatic shortcut via this natural kernel-contained N: FAIL / CLOSED.
- The result still does not claim that no other specially chosen N can yield a quotient-inheritance proof; such a universal non-implication is not needed.
