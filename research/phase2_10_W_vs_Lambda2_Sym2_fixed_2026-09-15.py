...
rank,R,piv=rref(E); free=[c for c in range(2025) if c not in piv]
basis=[]
for f in free:
 x=np.zeros(2025,dtype=np.int64);x[f]=1
 for row,c in enumerate(piv):x[c]=(-R[row,f])%P
 basis.append(x.reshape((45,45),order='F'))
rng=np.random.default_rng(20260915);maxrank=0
candidates=basis
for _ in range(512):
 coeff=rng.integers(0,3,size=len(basis)); candidates.append(sum((int(c)*b for c,b in zip(coeff,basis)),np.zeros((45,45),dtype=np.int64))%P)
for X in candidates:
 rr=rref(X)[0]
 if rr>maxrank:maxrank=rr
 if rr==45:break
...
print('dim Hom_H =',2025-rank);print('maximum rank among tested intertwiners =',maxrank)
print('FULL_RANK_INTERTWINER_FOUND =',maxrank==45)
if maxrank==45:print('CERTIFICATE: W ~= Lambda^2(Sym^2(V))')
else:print('NO FULL-RANK INTERTWINER FOUND; not a non-isomorphism proof.')
